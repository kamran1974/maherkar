    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            if email_exist:
                ...
                #     return redirect('register_employer',verify_code=new_user.verify_code)
                # elif new_user.user_type == 'jobseeker':
                #     return redirect('register_jobseeker',verify_code=new_user.verify_code)    
        ...
        if form.is_valid():
            ...
            if find_user.verify_code == active_code:
                ...
                if find_user.expire_date_code >now:
                    ...
                    elif find_user.user_type == 'jobseeker':
...
class Login(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            if find_user == None:
                form.add_error('email','حساب یافت نشد')
            else:
                check_password = User.check_password(find_user,password)
                if check_password == False:
                    form.add_error('email','ایمیل یا رمزعبور اشتباه است')
                else:
                    ...
                    if find_user.is_active:
                        ...
                        form.add_error('email',"حساب فعال نیست")
        context = {'form':form}
        return render(request,"account_module/login.html",context)
