        ...
        if request.user.is_authenticated:
            ...
            if get_user_type == "jobseeker":
                ...
                advertisments = advertisments.annotate(
                    near_by=Case(
                        When (county = get_user.jobseeker.county,job =get_user.jobseeker.job_category  , then=Value(1)),
                        When (province = get_user.jobseeker.province,job =get_user.jobseeker.job_category  , then=Value(2)),
                        When (job =get_user.jobseeker.job_category  , then=Value(3)),
                        When (county = get_user.jobseeker.county  , then=Value(4)),
                        When (province = get_user.jobseeker.province  , then=Value(5)),
                        default=Value(6),
    ...
    def post(self,request:HttpRequest):
        ...
        if form.is_valid():
            ...
            if job_type and job_type!="select":
                query &= Q(job__job_category__icontains=job_type)
            if title:
                query &=Q(title__icontains = title)
        
            advertisments = Advertisment.objects.filter(query)
